...
> **[Amendment GĐ7-recon — 2026-07-01]** *(bổ sung GĐ7-recon, xem `docs/deviations-log.md` — Bước 7)* Làm rõ định nghĩa "rỗng" mà đoạn trên để ngỏ. "Rỗng" = **không còn User nào trỏ `teamId`**, đếm CẢ user `isActive=false`. Lý do: `teamId` bất biến sau khi tạo (mục 9.5) và FK Restrict chặn theo **sự tồn tại của row** chứ không theo `isActive` — nên **vô hiệu hoá member KHÔNG giải phóng nhóm** (câu "dọn = vô hiệu hoá hết member" ở trên không đủ để xoá được một nhóm đã từng có member). Hệ quả thực tế: 204 chỉ đạt trên nhóm **chưa từng có member** (vd vừa `POST /teams`); nhóm còn bất kỳ user nào (active hay inactive) luôn trả 409 `TEAM_NOT_EMPTY`. Việc thực sự dọn sạch một nhóm có lịch sử member là thao tác chỉnh-dữ-liệu, để dành portfolio.
