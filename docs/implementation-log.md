...
## [GĐ11 slice 1] Redis làm store cho throttle — và một lỗi scale ẩn suốt GĐ10 — 2026-08-06
- Triệu chứng (cái đi sửa): `ThrottlerModule` không nhận `storage` nên bộ đếm nằm trong RAM một tiến trình. Chạy N replica thì giới hạn "5 login/phút/IP" nở thành 5 nhân N, và chặn **thất thường** — cùng một IP bị chặn ở request này rồi lọt ở request ngay sau, tuỳ rơi vào replica nào.
- **Bug thật tìm ra TRONG lúc verify (nghiêm trọng hơn cái đang sửa):** lượt đối chứng in-memory với 2 replica vẫn chặn đúng ở request thứ 6, y hệt một replica — đáng lẽ phải tới khoảng request thứ 10. Đo bằng một container nginx probe tách riêng, `log_format` chỉ in `$upstream_addr`: tên host viết thẳng → **8/8 request vào `172.22.0.4`**; `resolver` + biến → 3 vào `.4`, 5 vào `.6`. ⇒ suốt GĐ10, `--scale backend=N` tạo N container nhưng gần như **toàn bộ traffic đi vào đúng một cái**, im lặng hoàn toàn: không log, không lỗi, healthcheck vẫn xanh cả N. Nếu không có lượt đối chứng thì lượt đo "có Redis" cũng ra kết quả đẹp mà không chứng minh được gì.
- **Bài học phương pháp — giả thuyết đầu tiên SAI, và chỉ phép đo lật được nó:** ban đầu quy nguyên nhân cho `getaddrinfo` của musl trả thiếu địa chỉ, dựa trên `getent hosts backend` chỉ in một dòng trong khi `nslookup 127.0.0.11` ra hai. Quy kết này đã viết vào 5 file rồi mới đi kiểm lại. Kiểm bằng cách gọi thẳng `socket.getaddrinfo('backend', 3000)` trong image Alpine (musl) và Debian (glibc): **cả hai đều trả đủ hai địa chỉ**. `getent hosts` chỉ in dòng đầu — đó là thói quen hiển thị của công cụ, không phải giới hạn của resolver. Probe thứ hai: tắt đúng replica đang bị ghim thì 4/4 request tiếp theo vẫn 200 ⇒ nginx CÓ giữ địa chỉ thứ hai, chỉ dùng làm dự phòng chứ không chia phần traffic. Tài liệu nginx nói một tên phân giải ra nhiều địa chỉ thì round-robin, nên hành vi đo được **khác kỳ vọng từ tài liệu**; lý do bên trong nginx chưa truy tới cùng và đã ghi rõ là chưa truy, thay vì thay một giả thuyết sai bằng một giả thuyết sai khác. Kết luận vận hành không đổi vì nó dựa trên số đo chứ không dựa trên cơ chế: khả dụng vẫn còn (có đường dự phòng), chia tải thì không.
- Sửa: `resolver 127.0.0.11 valid=10s ipv6=off` + `set $itms_backend http://backend:3000` + `proxy_pass $itms_backend$request_uri`. **Biến trong `proxy_pass` mới là mấu chốt** — chỉ khi đích chứa biến thì nginx mới hoãn phân giải sang lúc chạy và mới dùng tới `resolver`; viết thẳng tên host là quay lại phân giải một lần lúc khởi động. `$request_uri` giữ URI thô kèm query string nên ba điều bắt buộc của `docs/10` §6 không suy suyển.
- Quyết định kỹ thuật (không hiển nhiên từ diff):
  - **Truyền URL dạng chuỗi vào `ThrottlerStorageRedisService`, KHÔNG tự dựng instance ioredis rồi truyền vào.** Adapter chỉ đặt cờ `disconnectRequired` cho client do chính nó tạo; đi đường chuỗi thì `onModuleDestroy` của adapter đóng kết nối giùm. Tự dựng instance là tự ôm lifecycle và rò handle lúc app tắt.
  - **`lazyConnect: true`** hoãn mở socket tới lệnh đầu. Bootstrap không phụ thuộc Redis sẵn sàng, và unit test dựng được store thật mà không cần Redis chạy (`redis.status === 'wait'` chính là assertion khoá điều đó).
  - **Khoá throttle vốn đã ổn định xuyên tiến trình, không phải cấu hình gì thêm:** `sha256("AuthController-login-default-" + req.ip)`. Tên class và handler nằm trong bundle nên hai replica sinh cùng khoá; `req.ip` là IP client thật nhờ `trust proxy` (Bước 7) cộng `X-Forwarded-For` của nginx (GĐ10). Hai mảnh đã có sẵn từ trước ăn khớp đúng chỗ.
- Verify: lint · `nest build` · unit **49/49** (tăng 4 case `throttler-storage.spec.ts`) · e2e **42/42** không đổi (`test/setup/env.ts` không đặt `REDIS_URL` nên lưới đi đường in-memory). Đa instance qua nginx `:8080`, 10 lần login sai mật khẩu, có **đối chứng**: in-memory ra `401×8 → 429 → 401` (giới hạn nở, chặn thất thường) so với Redis ra `401×5 → 429×5` (đúng 5/phút, chặn dứt khoát). Lượt đối chứng đóng hai vai — vừa cho thấy lỗi, vừa **chứng minh nginx thật sự chia tải cả hai replica** (nếu còn ghim thì đối chứng cũng phải chặn ở request 6). Phía Redis: `client list | grep -c ioredis` = 2, đúng một cặp khoá `{sha256:default}:hits`/`:blocked` dùng chung, `hits`=7, `pttl`≈58.8s. Hợp đồng nguyên vẹn: 429 + `Retry-After: 60` + envelope `code: RATE_LIMITED`. Rà lại `docs/10` sau khi động vào nginx, tất cả xanh: login thật qua proxy, cookie refresh vẫn `Path=/api/v1/auth`, refresh xoay vòng 200, query string giữ nguyên (`?page=2&limit=3` về đúng `meta.page`=2 `meta.limit`=3), `/api/v1/docs` 200, `/tasks` trả `index.html`.

---

## [GĐ11 slice 2] BullMQ đẩy email khỏi đường request — vòng import làm decorator nhận `undefined`, và đóng lỗ e2e gửi mail thật — 2026-08-16
- Triệu chứng (cái đi sửa): `POST /tasks` giao việc cho người khác mất **~2,75 giây**, gần như toàn bộ là vòng đi về tới SMTP. Task đã ghi xong từ lâu mà client vẫn phải chờ. `docs/07.A` §5 đã ghi sẵn đây là đánh đổi tạm, giữ vì "thêm hạ tầng" — lý do đó biến mất khi slice 1 đưa Redis vào compose.
- **Bug thật số 1 — vòng import làm decorator nhận `undefined`, app chết ở bootstrap.** Ban đầu gom cả hằng số (tên queue, tên job) lẫn hàm wiring vào một file `notification-queue.ts`. File đó import ba class `NotificationsProcessor` / `OverdueDigestSweep` / `OverdueDigestScheduler`, còn ba class đó import ngược lại hằng số từ nó. Hậu quả KHÔNG phải lỗi biên dịch: lúc `overdue-digest.scheduler.ts` được evaluate (đang nằm giữa quá trình load `notification-queue.ts`), `NOTIFICATION_QUEUE` chưa được gán, nên `@InjectQueue(undefined)` và `@Processor(undefined)` **âm thầm rơi về queue tên `default`**. App chết lúc bootstrap với `Nest can't resolve dependencies of the OverdueDigestScheduler (?, ConfigService) … "BullQueue_default"` — một thông báo trỏ vào đúng chỗ sai nhưng không nói gì về nguyên nhân. Sửa: tách `notification-queue.constants.ts` không import gì từ trong thư mục queue; file wiring import nó và ba class, ba class chỉ import file hằng. Bài học: file vừa giữ hằng-số vừa giữ wiring thì decorator nào dùng hằng số đó đều là bom hẹn giờ, và giá trị `undefined` trong decorator hỏng theo kiểu im lặng chứ không nổ tại chỗ.
- **Bug thật số 2 — đóng lại lỗ đã ghi ở entry 2026-08-05 (`delete` làm ngược điều nó định làm).** `test/setup/env.ts` dùng `delete process.env.MAIL_ENABLED` để tắt email trong lưới e2e. `ConfigModule.forRoot()` điền biến từ `.env` qua bộ lọc `!(key in process.env)`, tức **chỉ chừa ra key ĐÃ TỒN TẠI**. `delete` làm key biến mất nên dotenv thấy chỗ trống rồi điền `MAIL_ENABLED=true` của `.env` vào; e2e bind `EmailNotifier` thật và bắn mail qua Resend mỗi lần tạo task / reassign / deactivate. Tái dựng có đối chứng: `delete` → `MAIL_ENABLED="true"`, gán `'false'` → giữ `"false"`. Sửa: **gán** giá trị falsy thay vì xoá, và `REDIS_URL` cũng gán chuỗi rỗng cùng lý do — việc này khoá luôn đường Redis của throttle store slice 1 trong lưới e2e. Bài học chung: muốn khoá một biến khỏi `.env` thì phải cho nó TỒN TẠI với giá trị falsy; xoá là mở cửa chứ không phải đóng.
- Quyết định kỹ thuật (không hiển nhiên từ diff):
  - **Quyết định bật/tắt queue phải nằm ở METADATA MODULE, không hoãn được sang runtime** như slice 1 đã làm với throttle store. BullMQ không có bản in-memory, và `Queue` mặc định bật `enableOfflineQueue` cùng `maxRetriesPerRequest: null`, nên `queue.add()` lúc vắng Redis **treo vĩnh viễn** chứ không reject — đăng ký vô điều kiện là làm treo cả `start:dev` lẫn lưới e2e ngay lúc khởi tạo.
  - **`import 'dotenv/config'` là dòng đầu `src/main.ts`.** Metadata của `tasks.module.ts` dựng TRƯỚC khi `ConfigModule.forRoot()` của `app.module.ts` chạy dotenv, vì require chạy theo thứ tự import và decorator module con áp trước module cha. Không có dòng này thì `REDIS_URL` đặt trong `.env` bật được throttle store nhưng im lặng không bật queue — một biến, hai cơ chế hiểu khác nhau. Đã verify trên output biên dịch: `require("dotenv/config")` ở dòng 3, `require("./app.module")` ở dòng 6.
  - **Hai token `NOTIFIER` / `DIRECT_NOTIFIER`** cắt vòng lặp bằng cấu trúc: worker gọi `NOTIFIER` thì mỗi job xong lại đẻ một job y hệt. Cờ `rethrow` bám theo `isNotificationQueueEnabled` nên MỘT provider phủ đúng cả hai thế giới (queue tắt → người tiêu thụ là use-case → nuốt; queue bật → người tiêu thụ là worker → ném).
  - **`connection: { url }` chứ không tự dựng instance ioredis** — đúng bài học slice 1: thư viện tự tạo client thì cũng tự đóng nó lúc shutdown. bullmq v6 nhận `url` thẳng trong `RedisOptions` (`BaseOptions`), đã đọc typing để xác nhận thay vì đoán.
  - **Digest quét qua `TaskQueryPort.list({overdue:true})`, KHÔNG export `overduePredicate`.** Giữ đúng một đường vào định nghĩa OVERDUE. Export ra là có ngay hai đường vào cùng một luật, và hai đường thì sớm muộn cũng lệch.
- Verify: lint · `nest build` · unit **73/73** (tăng từ 49) · e2e **42/42** không đổi, cả hai lưới chạy KHÔNG cần Redis; `node dist/src/main` không Redis lên bình thường, không BullModule, không scheduler.
  - **Đường request, có đối chứng** (cùng stack, cùng `MAIL_ENABLED=true` trỏ Resend thật, chỉ khác đúng biến `REDIS_URL`): đồng bộ ra **2,922 · 2,750 · 2,712 giây**, queue ra **0,016 · 0,011 · 0,011 giây** — nhanh hơn ~250 lần. Lượt đối chứng là phần bắt buộc; chỉ đo lượt có queue thì số vẫn đẹp mà không chứng minh được nó đến từ đâu.
  - **Thư vẫn đi, và retry là thật:** worker nhận job, nói chuyện SMTP thật. Provider ở sandbox chỉ nhận địa chỉ chủ tài khoản nên trả 550 cho `@demo.local`; mỗi job hỏng xuất hiện **đúng ba lần** giãn dần rồi nằm lại `failed` — đó là `attempts: 3` + backoff hàm mũ đang chạy, và nó CHỈ chạy được vì adapter worker ném thay vì nuốt. Đối chứng chiều ngược lại trong cùng lượt: nhóm Backend có leader là hộp thư thật nên job digest **completed** và thư tới thật, nhóm Frontend (`fe.lead@demo.local`) thì `failed`. Cùng cơ chế, hai kết cục, khác nhau đúng ở địa chỉ người nhận.
  - **Lịch chạy một lần trên 2 replica:** `OVERDUE_DIGEST_CRON="*/1 * * * *"` với `--scale backend=2`, bốn chu kỳ liên tiếp — replica A chạy mốc 17:33 và 17:36, replica B chạy 17:34 và 17:35. **Mỗi mốc đúng một lần, không mốc nào ở cả hai**, đồng thời cả hai đều có lượt chạy nên cả hai là worker hợp lệ. `@Cron` thì mỗi mốc phải ra hai dòng. Phía Redis: zset lịch đúng MỘT entry `overdue-digest`.
  - Hợp đồng nguyên vẹn: login qua proxy, cookie refresh `Path=/api/v1/auth`, `?page=2&limit=3` về đúng `meta`, `/api/v1/docs` 200, SPA fallback 200. `GET /stats` ra `total=6 overdue=4`, `byProgress` vẫn 3 key và `overdue` vẫn là sibling.
