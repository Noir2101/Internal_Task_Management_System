...
- **Slice 2 — BullMQ async email + digest quá hạn ✅:** việc gửi email rời khỏi đường request sang worker, gắn SAU seam `Notifier` nên `CreateTask`/`ReassignTask`/`Users.deactivate` không sửa dòng nào · `QueuedNotifier` (decorator) ghi job, `NotificationsProcessor` gửi qua `DIRECT_NOTIFIER` — tách hai token để worker không tự đẻ job · hook thứ tư `notifyOverdueDigest` trên port, quét qua `TaskQueryPort.list({overdue:true})` (KHÔNG export `overduePredicate`) · lịch = **repeatable job của BullMQ chứ không `@nestjs/schedule`** (`@Cron` bắn trên mọi replica ⇒ leader nhận N bản trùng), `OVERDUE_DIGEST_CRON` mặc định 01:00 UTC · queue bật/tắt theo `REDIS_URL` quyết ở metadata module (BullMQ không có bản in-memory, `queue.add` lúc vắng Redis TREO chứ không reject) + `dotenv/config` đầu `main.ts` cho `.env` có hiệu lực · cờ `rethrow` để lỗi gửi nổi lên cho BullMQ retry ở đường worker, đường use-case giữ nguyên nuốt lỗi · **fix lưới e2e gửi mail thật**: `test/setup/env.ts` GÁN `MAIL_ENABLED='false'` thay vì `delete` (dotenv chỉ chừa key đã tồn tại ⇒ xoá là mời `.env` điền vào) · verify có đối chứng: `POST /tasks` **2,75 giây → 11 mili giây**, và `--scale backend=2` ra 4 chu kỳ mỗi mốc chạy **đúng một lần** · unit 49→**73**, e2e 42 không đổi, cả hai không cần Redis.
- **Slice 3–4 — phác ở `docs/11 §8`:** structured logging pino · audit log append-only.
